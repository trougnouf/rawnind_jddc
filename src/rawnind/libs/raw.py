...
try:
    ...
    cv2 = None  # Optional dependency for some utility functions
...
def demosaic(
    ...
    if is_bayer:
        ...
        pattern = "xtrans"
        layout = ""  # X-Trans doesn't use layout parameter
        # Pass algorithm through - OIIO may support additional algorithms for X-Trans
        # beyond those documented (e.g., markesteijn variant)
    
    # No white balance adjustment (neutral 1.0, 1.0, 1.0)
    # White balance should be applied during color correction, not demosaic
    white_balance = (1.0, 1.0, 1.0)
    
    dst_buf = oiio.ImageBufAlgo.demosaic(
        src_buf, 
        pattern=pattern,
        layout=layout,
        algorithm=algorithm,
...
def markesteijn_demosaic(mono_img: np.ndarray, metadata: dict) -> np.ndarray:
    """x-trans demosaicing using Markesteijn 3-pass algorithm.
    ...
    if not (6 <= cstat[0] <= 10 and 16 <= cstat[1] <= 24 and 
            6 <= cstat[2] <= 10 and cstat[3] == 0):
    ...
    for row in range(3, height - 3):
        for col in range(3, width - 3):
            ...
            color_0 = (174 * (get_pix_at_offset(1, 1) + get_pix_at_offset(1, 0)) -
                       46 * (get_pix_at_offset(2, 1) + get_pix_at_offset(2, 0))) / 256.0
            
            # Direction 1: another h/v variant using hex[2] and hex[3]
            # Includes current pixel value (f channel) for correction
            color_1 = (223 * get_pix_at_offset(1, 3) + 
                       33 * get_pix_at_offset(1, 2) +
                       92 * (mono_img[row, col] - get_pix_at_offset(-1, 2))) / 256.0
            
            # Directions 2 and 3: diagonal interpolations using hex[4] and hex[5]
            # Uses ±2x and ±3x distances for longer-range edge detection
            color_2 = (164 * get_pix_at_offset(1, 4) + 
                       92 * get_pix_at_offset(-2, 4) +
                       33 * (2 * mono_img[row, col] - 
                             get_pix_at_offset(3, 4) - 
                             get_pix_at_offset(-3, 4))) / 256.0
            
            color_3 = (164 * get_pix_at_offset(1, 5) + 
                       92 * get_pix_at_offset(-2, 5) +
                       33 * (2 * mono_img[row, col] - 
                             get_pix_at_offset(3, 5) - 
                             get_pix_at_offset(-3, 5))) / 256.0
    ...
    print(f"Homogeneity selection complete")
    ...
    try:
        ...
        if check_exposure and not is_exposure_ok(img, metadata):
            ...
            return ConversionResult(
                ConversionOutcome.BAD_EXPOSURE, src_fpath, dest_fpath
            )
