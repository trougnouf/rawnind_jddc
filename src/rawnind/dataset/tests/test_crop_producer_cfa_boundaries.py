...
def mock_bayer_scene():
    ...
    noisy_img.metadata = {
        "alignment": [3, 5],
        "gain": 1.0,
    }  # Odd alignment - should be snapped
...
def mock_xtrans_scene():
    ...
    noisy_img.metadata = {
        "alignment": [4, 7],
        "gain": 1.0,
    }  # Not multiples of 3 - should be snapped
    noisy_img.sha1 = "efgh5678"
...
def test_bayer_crop_positions_are_even(temp_output_dir, mock_bayer_scene):
    ...
    noisy_data = (
        gt_data + np.random.randn(h, w).astype(np.float32) * 0.05
    )  # Add correlated noise

    stage = CropProducerStage(
        output_dir=temp_output_dir, crop_size=256, num_crops=10  # Even
    )
...
def test_xtrans_crop_positions_multiple_of_3(temp_output_dir, mock_xtrans_scene):
    ...
    stage = CropProducerStage(
        output_dir=temp_output_dir, crop_size=255, num_crops=10  # Multiple of 3
    )
...
def test_bayer_alignment_offsets_snapped_to_even(temp_output_dir, mock_bayer_scene):
    ...
    stage = CropProducerStage(output_dir=temp_output_dir, crop_size=256, num_crops=1)
...
def test_xtrans_alignment_offsets_snapped_to_multiple_of_3(
    temp_output_dir, mock_xtrans_scene
):
    ...
    stage = CropProducerStage(output_dir=temp_output_dir, crop_size=255, num_crops=1)
...
def test_mask_png_saved_to_disk(temp_output_dir, mock_bayer_scene):
    ...
    noisy_img.metadata.update(
        {"alignment": [0, 0], "gain": 1.0, "overexposure_lb": 0.99, "is_bayer": True}
    )
    ...
    noisy_data = (
        gt_data + np.random.randn(3, h, w).astype(np.float32) * 0.05
    )  # Add noise
    ...
    expected_mask_path = (
        temp_output_dir / "masks" / f"{scene.scene_name}_{noisy_img.sha1[:8]}_mask.png"
    )
...
def test_mask_fpath_added_to_crop_metadata(temp_output_dir, mock_bayer_scene):
    ...
    stage = CropProducerStage(output_dir=temp_output_dir, crop_size=256, num_crops=3)
    ...
    for crop in crop_metadata:
        ...
        assert mask_path.name.endswith(
            "_mask.png"
        ), f"Invalid mask filename: {mask_path.name}"
...
def test_vectorized_batch_validation_finds_valid_crops(
    temp_output_dir, mock_bayer_scene
):
    ...
    stage = CropProducerStage(
        output_dir=temp_output_dir, crop_size=256, num_crops=5  # Request 5 crops
    )
    ...
    for crop in crop_metadata:
        ...
        if y == 0 and x == 0:
            ...
            pytest.fail("Crop at (0,0) should have been rejected due to masking")
...
def test_bayer_prgb_crops_saved_as_exr(temp_output_dir, mock_bayer_scene):
    ...
    for crop in crop_metadata:
        ...
        bayer_gt_path = (
            temp_output_dir / "bayer" / scene.scene_name / f"{crop_id}_gt.npy"
        )
        ...
        prgb_noisy_path = (
            temp_output_dir / "prgb" / scene.scene_name / f"{crop_id}_noisy.exr"
        )
...
def test_prgb_crops_have_correct_shape_and_dtype(temp_output_dir, mock_bayer_scene):
    ...
    stage = CropProducerStage(
        output_dir=temp_output_dir, crop_size=256, num_crops=1, crop_types=["prgb"]
    )
...
def test_xtrans_prgb_crops_work_with_markesteijn(temp_output_dir, mock_xtrans_scene):
    ...
    assert (
        prgb_noisy_path.exists()
    ), f"x-trans PRGB noisy crop not found: {prgb_noisy_path}"
