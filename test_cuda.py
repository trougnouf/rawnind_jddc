#!/usr/bin/env python3
"""
Quick CUDA functionality test script
"""
...
def test_cuda():
    ...
    try:
        ...
        print(f"✗ Device properties failed: {type(e).__name__}: {e}")
        return False
    ...
    # Test 6: FFT operations (used in alignment)
    try:
        from cupyx.scipy.signal import correlate as cp_correlate
        a = cp.random.random((100, 100))
        b = cp.random.random((100, 100))
        corr = cp_correlate(a, b, mode='same')
        print(f"✓ FFT correlation test: result shape {corr.shape}")
    except Exception as e:
        print(f"✗ FFT correlation failed: {type(e).__name__}: {e}")
        return False
