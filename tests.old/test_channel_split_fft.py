...
for test_case in test_dataset["test_cases"]["misaligned_bayer"]:
    ...
    error_standard = abs(fft_standard[0] - test_case["expected_shift"][0]) + abs(
        fft_standard[1] - test_case["expected_shift"][1]
    )
    match_standard = (
        "✅" if fft_standard == tuple(test_case["expected_shift"]) else "❌"
    )
    print(
        f"    {match_standard} Shift: {fft_standard}, Loss: {loss_standard:.6f}, Error: {error_standard}px"
    )

    # Channel-split FFT with median
    print("  [2/3] Channel-split FFT (median)...")
    fft_split_median, channel_shifts = fft_phase_correlate_channel_split(
    ...
    error_split_median = abs(
        fft_split_median[0] - test_case["expected_shift"][0]
    ) + abs(fft_split_median[1] - test_case["expected_shift"][1])
    match_split_median = (
        "✅" if fft_split_median == tuple(test_case["expected_shift"]) else "❌"
    )
    print(
        f"    Per-channel shifts: R={channel_shifts[0]}, G1={channel_shifts[1]}, G2={channel_shifts[2]}, B={channel_shifts[3]}"
    )
    print(
        f"    {match_split_median} Median shift: {fft_split_median}, Loss: {loss_split_median:.6f}, Error: {error_split_median}px"
    )

    # Channel-split FFT with mean
    print("  [3/3] Channel-split FFT (mean)...")
    fft_split_mean, channel_shifts = fft_phase_correlate_channel_split(
        gt_img, noisy_matched, method="mean"
    )
    ...
    error_split_mean = abs(fft_split_mean[0] - test_case["expected_shift"][0]) + abs(
        fft_split_mean[1] - test_case["expected_shift"][1]
    )
    match_split_mean = (
        "✅" if fft_split_mean == tuple(test_case["expected_shift"]) else "❌"
    )
    print(
        f"    {match_split_mean} Mean shift: {fft_split_mean}, Loss: {loss_split_mean:.6f}, Error: {error_split_mean}px"
    )
    ...
    print(
        f"    Channel-mean:   Error={error_split_mean}px {'🎯 BETTER' if error_split_mean < error_standard else ''}"
    )
    print()
