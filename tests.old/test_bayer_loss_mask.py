...
def test_bayer_vs_rgb_pipeline():
    ...
    for image_set in os.listdir(bayer_ds_path):
        ...
        noisy_files = [f for f in os.listdir(set_path) if f.endswith('.dng') and not f.startswith('gt')]
    ...
    for idx, test_set in enumerate(test_sets):
        ...
        # Load images
        gt_path = os.path.join(bayer_ds_path, test_set['image_set'], 'gt', test_set['gt_file'])
        noisy_path = os.path.join(bayer_ds_path, test_set['image_set'], test_set['noisy_file'])
        ...
        bayer_alignment, bayer_loss = find_best_alignment_fft_cfa(
            gt_img, noisy_img, gt_metadata,
            method="median", return_loss_too=True, verbose=False
        )
        print(f"    Alignment: {bayer_alignment}, Loss: {bayer_loss:.6f}")
        
        # Shift Bayer images
        gt_bayer_aligned, noisy_bayer_aligned = shift_images(gt_img, noisy_img, bayer_alignment)
        overexp_mask_bayer = shift_mask(overexp_mask, bayer_alignment)
        
        # Compute loss mask on Bayer data
        loss_mask_bayer = make_loss_mask_bayer(gt_bayer_aligned, noisy_bayer_aligned) * overexp_mask_bayer
        ...
        print(f"    Loss mask (Bayer): mean={loss_mask_bayer.mean():.4f}, "
        ...
        rgb_alignment, rgb_loss = find_best_alignment(
            gt_rgb, noisy_rgb, 
            return_loss_too=True, method="fft", verbose=False
        )
        print(f"    Alignment: {rgb_alignment}, Loss: {rgb_loss:.6f}")
        
        # Shift RGB images
        gt_rgb_aligned, noisy_rgb_aligned = shift_images(gt_rgb, noisy_rgb, rgb_alignment)
        overexp_mask_rgb = shift_mask(overexp_mask, rgb_alignment)
        
        # Compute loss mask on RGB data
        loss_mask_rgb = make_loss_mask(gt_rgb_aligned, noisy_rgb_aligned) * overexp_mask_rgb
        
        print(f"    Loss mask (RGB): mean={loss_mask_rgb.mean():.4f}, "
        ...
        if not alignment_match:
            ...
            print(f"      Difference: ({bayer_alignment[0]-rgb_alignment[0]}, {bayer_alignment[1]-rgb_alignment[1]})")
        ...
        if gt_rgb_from_bayer.shape == gt_rgb_aligned.shape:
            ...
            print(f"    Noisy image MAE: {noisy_mae:.6f}, Max error: {noisy_max_error:.6f}")
        else:
        ...
        if loss_mask_bayer.shape == loss_mask_rgb.shape:
            ...
            print(f"    Pixels accepted by Bayer but rejected by RGB: {bayer_only.sum()}")
    ...
    if valid_mask_comparisons:
        ...
        avg_agreement = np.mean([r['mask_agreement_pct'] for r in valid_mask_comparisons])
        print(f"\nLoss masks:")
        print(f"  Average MAE: {avg_mask_mae:.6f}")
        print(f"  Average agreement: {avg_agreement:.2f}%")
        for r in valid_mask_comparisons:
            print(f"    {r['image_set']}: MAE={r['mask_mae']:.6f}, "
