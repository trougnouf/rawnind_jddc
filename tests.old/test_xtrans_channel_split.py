...
def extract_xtrans_channels(img):
    ...
    return (
        (R_mask, R_values, R_positions),
        (G_mask, G_values, G_positions),
        (B_mask, B_values, B_positions),
    )
...
def create_dense_channel_image(mask, values, shape):
    ...
    for i in range(sampled_h):
        for j in range(sampled_w):
            ...
            block_values = values[
                block_y : min(block_y + 6, h), block_x : min(block_x + 6, w)
            ]
            block_mask = mask[
                block_y : min(block_y + 6, h), block_x : min(block_x + 6, w)
            ]
...
for scene_name in test_scenes:
    ...
    try:
        ...
        print(
            f"    Camera: {gt_meta.get('make', 'Unknown')} {gt_meta.get('model', 'Unknown')}"
        )
