    ...
    while improved and (abs(best_shift[0]) + abs(best_shift[1]) < max_shift):
        ...
        for dy in range(-neighborhood, neighborhood + 1):
            for dx in range(-neighborhood, neighborhood + 1):
                ...
                try:
                    anchor_crop, candidate_crop = shift_images(anchor_img, candidate_img, test_shift)
...
test_cases = [
    ("Bark", "Bayer_Bark_GT_ISO100_sha1=f15da1140d949ee30c15ce7b251839a7b7a41de7.cr2",
     "Bayer_Bark_ISO65535_sha1=6ba8ed5f7fff42c4c900812c02701649f4f2d49e.cr2", (12, -10)),
    ("Bark", "Bayer_Bark_GT_ISO100_sha1=f15da1140d949ee30c15ce7b251839a7b7a41de7.cr2",
     "Bayer_Bark_ISO800_sha1=ba86f1da64a4bb534d9216e96c1c72177ed1e625.cr2", (4, -4)),
    ("Kortlek", "Bayer_Kortlek_GT_ISO100_sha1=b5a564cd9291224caf363b6b03054365d59d316b.cr2",
     "Bayer_Kortlek_ISO51200_sha1=8fed453fdfb162673bb9ed41f9c5f03095331e3b.cr2", (0, -4)),
]
...
for scene, gt_name, noisy_name, expected_shift in test_cases:
    ...
    fft_error = abs(fft_shift[0] - expected_shift[0]) + abs(fft_shift[1] - expected_shift[1])
    
    print(f"  Simple:     {simple_match} Shift: {simple_shift}, Loss: {simple_loss:.6f}")
    print(f"  FFT (FIXED): {fft_match} Shift: {fft_shift}, Loss: {fft_loss:.6f}, Error: {fft_error}px")
    print()
